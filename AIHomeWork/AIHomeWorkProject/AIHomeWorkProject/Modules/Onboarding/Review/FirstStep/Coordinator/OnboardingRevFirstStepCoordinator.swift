//
//  OnboardingRevFirstStepCoordinator.swift
//  AIHomeWorkProject
//
//  Created by George Popkich on 27.08.24.
//

import Foundation

final class OnboardingRevFirstStepCoordinator: Coordinator {
    
    private var rootVC: OnboardingRevFirstStepVC?
    private let container: Container
    private let onboardingType: Onboarding.Options
    
    init(container: Container,  onboardingType: Onboarding.Options) {
        self.container = container
        self.onboardingType = onboardingType
    }
    
    override func start() -> OnboardingRevFirstStepVC {
        let onboardingRevFirstStepVC =
        OnboardingRevFirstStepAssembler.make(coordinator: self)
        rootVC = onboardingRevFirstStepVC
        return onboardingRevFirstStepVC
    }
    
}

extension OnboardingRevFirstStepCoordinator:
    OnboardingRevFirstStepCoordinatorProtocol {
    
    func nextStep() {
        let coordinator = OnboardingRevSecondStepCoordinator(
            container: container,
            onboardingType: onboardingType)
        self.addChildCoordinator(coordinator)
        
        let onboardingRevSecondStepVC = coordinator.start()
        
        coordinator.onDidFinish = { [weak self] coordinator in
            self?.removeChildCoordinator(coordinator)
            self?.finish()
            onboardingRevSecondStepVC.dismiss(animated: true)
        }
        
        onboardingRevSecondStepVC.modalPresentationStyle = .fullScreen
        rootVC?.presentDetail(onboardingRevSecondStepVC, 0.40)
    }
    
}
